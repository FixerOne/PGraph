...
import (
	controller "pgraph/controller/testquestion"
...
func (h *handler) StartHandlers() {
	...
	h.server.GET("/testQuestion/GetAll", func(c *gin.Context) {
		headers.SetUpHeaders(c)
		c.JSON(200, Controller.FindAll())
	})

	h.server.GET("/testQuestion/GetByTestType/:id", func(c *gin.Context) {
		headers.SetUpHeaders(c)
		id := c.Param("id")
